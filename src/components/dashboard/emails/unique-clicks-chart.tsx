...
const UniqueClicksChart = () => {
    ...
    const formattedData = getStats.data?.map((item: any) => {
        return {
            name: item.date,
            ["Unique Clicks"]: item.stats[0].metrics.deferred,
        };
    });
        ...
        <Card className="h-[450px] w-full shadow">
            <CardHeader className="flex flex-row items-center">
                <div className="grid gap-2">
                    <CardTitle>Unique Clicks</CardTitle>
                    <div>
                        <CardDescription>Unique clicks over time</CardDescription>
                    </div>
                </div>
            </CardHeader>
...
export default UniqueClicksChart;
