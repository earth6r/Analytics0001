    ...
    for (const phoneCall of phoneCalls) {
        ...
        if (minutesUntilTour === 30 && Object.keys(interviewers).includes(phoneCall?.interviewer || '')) {
            ...
            await sendNotification(
                interviewerPhone,
                `REMINDER: You have a call with ${phoneCall.firstName} ${phoneCall.lastName} in 30min. Check the profile here: https://home0001.com/booking-details?email=${phoneCall.email}&type=Phone Call&uid=${phoneCall.uid}`,
            );
            ...
            await sendNotification(
                interviewerPhone,
                `REMINDER complete the post-meeting notes for your call with ${phoneCall.firstName} ${phoneCall.lastName}. You can leave your notes here: https://home0001.com/booking-details?email=${phoneCall.email}&type=Phone Call&uid=${phoneCall.uid}`,
            );
        }
    }
